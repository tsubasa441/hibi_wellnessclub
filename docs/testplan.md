...
| 4-6 | PayPay を選択し決済を実行 | PayPayアプリへの遷移→支払い完了→予約確定（サイトへ戻る場合も、戻らず `/bookings` 等を開く場合も）→確認メール→キャンセルで PayPay 側へ返金 | `paypay/route.test.ts`, `paypay/callback/route.test.ts`, `lib/paypayReconcile.test.ts`, `cancel/route.test.ts` | 🟡 一部確認・保留中（2026-09-19 本番E2Eの途中）。**確認済み**：本番の環境変数を Production に設定（当初は Development にしか入っておらず `QRCodeCreate` が `codeId 08100016` で失敗）→QuotaGuard 経由で決済URL発行→PayPay アプリで ¥100 の支払い完了（PayPay 加盟店管理画面で「完了」）。**未確認（不具合修正後の本番再検証が必要）**：(a) 予約の確定（コールバック経由／`/bookings` 等のロード時の照会経由）、(b) 確認メール、(c) キャンセルの PayPay 返金（新しい `PaymentRefund` 呼び出し）、(d) 未払い予約の自動解放。初回の実決済では、支払い済みなのに予約が確定せず、テスト予約が削除された（BUG-13）。手順は `docs/deployment.md` の「PayPay 本番化の手順・注意点」参照。PayPay 側にはテスト決済 ¥100 が「完了」のまま残っている（返金するかは依頼者判断）
