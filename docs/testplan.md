...
| 1-8 | 送信されたリセットメールのリンクから `/auth/reset-password` を開き新パスワードを設定 | 再設定完了画面→「ログイン画面へ」で `/login` に遷移しログイン可能 | - | 🟡 2026-08-22時点ではBUG-3修正後の旧実装（`?code=`方式）で本番確認済みだったが、その後 BUG-9（2026-09-01）・BUG-10（2026-09-05）でパスワード再設定の仕組み自体を作り替えたため無効。2026-09-05の本番再検証で実際にリセットメールのリンクを開いたところ BUG-10 を発見・修正した（`docs/testplan.md` BUG-10参照、`npm run test`/lint/build は通過確認済み）が、**修正後の新しいリンクでの本番再検証（実メール・別デバイスでの疎通確認）はデプロイ待ちのため未実施** |
