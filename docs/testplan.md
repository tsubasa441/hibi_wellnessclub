...
| # | 確認項目 | 期待結果 | 自動テスト | 状態 |
|---|---------|---------|-----------|------|
| 18-1 | 各ページ（`/login`（ログイン・新規登録）、`/register-complete`、`/legal/tokushoho`、`/legal/privacy`、`/legal/terms`）の上部 | トップと同じヘッダー（nav-bg 背景・「Hibi」ロゴ）が表示され、ロゴを押すと `/` へ遷移する | - | 未確認 |
| 18-2 | トップページのヘッダー | 従来と同じ見た目（共通コンポーネントへの置き換えで変わらない） | - | 未確認 |
