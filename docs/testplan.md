...
| 4-6 | PayPay を選択し決済を実行 | PayPayアプリへの遷移→支払い完了→予約確定（サイトへ戻る場合も、戻らず `/bookings` 等を開く場合も）→確認メール→キャンセルで PayPay 側へ返金 | `paypay/route.test.ts`, `paypay/callback/route.test.ts`, `lib/paypayReconcile.test.ts`, `cancel/route.test.ts` | 🟡 一部確認・保留中。**確認済み**：本番の環境変数設定・QuotaGuard経由の決済URL発行・PayPayアプリでの支払い完了（以上2026-09-19）。**2026-09-23 追加確認**：`userAgent` 転送（`e6ee979`）・決済URLを新しいウィンドウで開く修正（`5911073`、BUG-13(6)参照）後、依頼者がPayPayアプリ経由で実決済を行い、支払い後にHibiへ自動で戻ること・`/bookings`が`paid`で確定することを確認。**未確認**：(b) 確認メールが届いたかの明示的な確認、(c) キャンセルの PayPay 返金（新しい `PaymentRefund` 呼び出し）、(d) 未払い予約の自動解放。手順は `docs/deployment.md` の「PayPay 本番化の手順・注意点」参照
