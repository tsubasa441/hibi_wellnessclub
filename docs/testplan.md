...
| 5-6 | キャンセル時、付与済みイベント参加ポイントがある場合 | `points_log` から削除、`profiles.points` から減算（0未満にならない） | `points.test.ts` | 🟡 保留。`checkAndAwardPendingPoints`のポイント付与条件と`cancel/route.ts`のキャンセル可否判定が共に`start_at + 2時間 <= now`を使っており同一条件のため、「参加ポイント付与済み・かつキャンセル可能」という状況が正規のUI操作では発生し得ない可能性が高い（`revokeEventPoints`呼び出しが実質到達不能では、という疑問点として次回以降に判断） |
...
| 8-2 | 月初、`CRON_SECRET` 付きで `/api/cron/badges` を実行 | 前月の月間バッジ獲得数に応じたボーナス（3個:300pt / 5個:500pt / 9個:1000pt、最高ティアのみ）が付与される | - | 🟡 2026-08-26（正しいSecretで実行し200が返ること、ポイントティア計算3/5/9個→300/500/1000ptは`points.test.ts`の`awardBadgeCountPoints`テスト7パターンで確認。ただし「月初(1日)のみ実行」の日付ゲート自体は本日が1日でないため実地では再現できていない） |
