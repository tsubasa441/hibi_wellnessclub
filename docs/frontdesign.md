...
- おしゃれさと親しみやすさの両立
- 色は 3 色に絞る（ベース × インク × セージグリーン）＋ ナビゲーション専用のアクセント1色
...
@theme {
  ...
  /* Navigation: Header・BottomNav 専用背景（2026-09、参考UIに合わせて追加）。
     ニューモーフィズムの光/影はこの背景自体から派生させる */
  --color-nav-bg:    #FAFAEC;
...
| ナビゲーション背景（Header・BottomNav） | `--color-nav-bg` | #FAFAEC |
| ナビゲーション内テキスト | `--color-ink-700` | #1C221F（可読性優先で全テキスト黒系に統一） |
