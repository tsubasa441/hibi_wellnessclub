...
### journals（2026-09-13 現フェーズ対象外：機能停止中。テーブル・既存データは保持、新規収集なし）
...
|-----------|------------|--------------|------|
| ~~ジャーナル回答（1日1回）~~ | ~~3pt~~ | 2026-09-13 機能停止中（次フェーズで再実装予定） | - |
...
| Step4 | ポイント・バッジ | ✅ 完了 |
| Step5 | 紹介コード・報酬付与 | ✅ 完了 |
| Step6 | ジャーナル機能（当日記録のみ） | 🛑 2026-09-13 機能停止（次フェーズで再実装予定） |
| Step7 | テスト・Vercel デプロイ・本番切り替え | 進行中（Vercel連携済み。Square 本番反映済み・実決済/返金を本番で確認（2026-09-03）。PayPay は本番の環境変数・固定IPプロキシ設定済みで決済の成立まで確認済み（2026-09-19）。修正後の予約確定・キャンセル返金の本番再検証は保留中。@docs/testplan.md の 4-6・BUG-13 参照） |
